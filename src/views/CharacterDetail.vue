<template>
  <div class="mx-auto" v-if="char">
    <button
      @click="goBack()"
      class="flex items-center space-x-1 border-b border-transparent hover:border-white"
    >
      <icon-mdi-arrow-left />
      <p>Go back to the list</p>
    </button>
    <h1 class="mt-1 mb-8 text-3xl sm:text-5xl">
      <span class="text-white text-opacity-50">#{{ char.id }} </span> {{ char.name }}
    </h1>
    <div class="block sm:(flex space-x-8 items-center)">
      ...
      <div class="grid grid-cols-1 gap-1 sm:(grid-cols-2 gap-4) md:grid-cols-3 w-max">
        <div>
          <label class="text-sm text-white text-opacity-50">Species</label>
          <p class="text-xl">{{ char.species || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Origin</label>
          <p class="text-xl">{{ char.origin.name || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Location</label>
          <p class="text-xl">{{ char.location.name || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Gender</label>
          <p class="text-xl">{{ char.gender || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Status</label>
          <p class="text-xl">{{ char.status || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Type</label>
          <p class="text-xl">{{ char.type || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Episode</label>
          <p class="text-xl">{{ char.episode.length || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-sm text-white text-opacity-50">Created</label>
